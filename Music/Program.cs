...
namespace Music
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                s._listOfAlbums = new List<Album>();
                foreach (var albumSong in albumSongs)
                {
                    if (albumSong.AlbumId != s.SongId) continue;
                    foreach (var al in albums)
                    {
                        if (albumSong.SongId == al.AlbumId)
                            s._listOfAlbums.Add(al);
                    }
            ...
            foreach (var album in groupAlbum)
            {
                foreach (var artist in artists)
                {
                    if(album.ArtistId == artist.ArtistId)
                        Console.WriteLine($"Album:  {album.Name} - Artist: {artist.Name} - Year: {album.YearOfIssue}");
                }
            }
            ...
            {
                Console.WriteLine($"Name of album: {album}");
            }
            //5.whole duration 
            Console.WriteLine("\nDuration of albums: ");
            foreach (var album in albums)
